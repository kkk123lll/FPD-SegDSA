...
# from fpd_seg.common.data_augmentation import Compose, ToTensor, CropToFixed, HorizontalFlip, VerticalFlip, RandomRotate90
# import cv2
...
#         self.gt_DA = Compose([
#             CropToFixed(np.random.RandomState(seed), size=self.size),  # 裁剪成64*64的大小
#             HorizontalFlip(np.random.RandomState(seed)),  # 进行水平翻转
#             VerticalFlip(np.random.RandomState(seed)),  # 进行垂直翻转
#             RandomRotate90(np.random.RandomState(seed)),  # 旋转90度
#             ToTensor(False)  # 转换为张量，False表示不进行归一化
#         ])

#     def __getitem__(self, idx):
#         id = np.random.randint(len(self.image_list))
#         img_id = self.image_list[id]
#         img =  np.load(os.path.join(self.images_path,img_id))  # img为一个npy文件（由一个序列打包而成）
#         gt = cv2.imread(os.path.join(
#                 self.labels_path, f"label_s{img_id.split('.')[0]}.png"), 0)
#         gt = np.array(gt/255)[np.newaxis]  # 进行归一化，然后在第0维添加一个新轴
...
#     def __getitem__(self, idx):
#         # print(len(self.img_patch))
#         # print(len(self.gt_patch))
...
class Test_dataset(Train_dataset):
    def __init__(self, config, images_path, labels_path):
        ...
        self.img_patches = []
        self.gt_patches = []
        self.patch_counts = []  # ⭐ 每张图像对应多少 patch

        for img, gt in zip(self.img_list, self.gt_list):
            img_patch = self.get_patch([img], self.patch_size, self.stride)
            gt_patch = self.get_patch([gt], self.patch_size, self.stride)

            self.patch_counts.append(len(img_patch))
            self.img_patches.extend(img_patch)
            self.gt_patches.extend(gt_patch)
    ...
    def get_patch(self, image_list, patch_size, stride):
        ...
        for image in image_list:
            ...
            # ✅ 正确的 padding 公式（必须有 % stride）
            pad_h = (stride - (h - patch_size[0]) % stride) % stride
            pad_w = (stride - (w - patch_size[1]) % stride) % stride
            ...
            image = image.unfold(1, patch_size[0], stride)\
    ...
    def __getitem__(self, idx):
        return self.img_patches[idx], self.gt_patches[idx].long()

    def get_patch_counts(self):
        return self.patch_counts
